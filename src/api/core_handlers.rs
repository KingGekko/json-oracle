...
use super::file_streaming::JsonStreamManager;
...
pub async fn ollama_process_json(
    ...
    let prompt_future = spawn_blocking(move || {
        // Use custom prompt or default generic AI analyst prompt
        let base_prompt = if payload_prompt.trim().is_empty() {
            "You are an AI data analyst specializing in pattern recognition and actionable insights. 

ANALYZE THE FOLLOWING DATA AND PROVIDE COMPREHENSIVE ANALYSIS:

REQUIRED OUTPUT FORMAT:

1. DATA SUMMARY:
   - Key findings and patterns in the dataset
...
Provide your response in a structured format with clear sections and specific actionable recommendations. Focus on data-driven insights and practical applications."
        } else {
            &payload_prompt
        };
...
pub async fn multi_model_conversation(
    ...
    let base_prompt = if payload.initial_prompt.trim().is_empty() {
        "You are an AI data analyst. Analyze the following data and provide comprehensive insights:"
    } else {
    ...
    let summary_prompt = format!(
        "Summarize this multi-model conversation about the data analysis. \
...
mod tests {
    ...
    async fn test_health_check() {
        ...
        assert_eq!(body["service"], "ai-json-analysis-api");
    }
